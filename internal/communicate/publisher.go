...
type Publisher struct {
	mu        sync.Mutex
	conn      *nats.Conn
	producers map[string]*natsclient.Producer
}

func NewPublisher(nc *nats.Conn) (*Publisher, error) {
	p := &Publisher{
		conn:      nc,
		producers: make(map[string]*natsclient.Producer, 0),
	}
...
func (p *Publisher) getProducer(subject string) (*natsclient.Producer, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	pr, err := natsclient.NewProducer(p.conn, subject)
	if err != nil {
